...
- [ ] **Seed assets bucket** — upload sample GLBs to `seeley-assets-{acct}` (Poly Haven / Sketchfab free tier).
...
npx cdk bootstrap
npx cdk deploy seeley-network seeley-ai   # start SageMaker warm-up first
npx cdk deploy seeley-data seeley-auth
npx cdk deploy seeley-api seeley-pipeline
```
